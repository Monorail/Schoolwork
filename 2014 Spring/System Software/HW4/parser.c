...
lexNode* block(lexNode* list, int indexT);
...
lexNode* block(lexNode* list, int indexT){//1 for int, 2 for const, 3 for proc (-1 for empty)
	if(PARSERDBG) if(aArg) printf("Parser: block(): start\n");
	int varCtr = 0;
	int ctempBlock = cx;
	int keepOwnIndex;
	int indexTemp = indexT;
	keepOwnIndex = indexTemp;
	emit(JMP, 0, 0 , 0);
	...
	while(list->tokType == procsym){
		...
		addProcSymbol(list->lex, lexLevel, cx);
		indexTemp = lookUp(list->lex);
	...
	assemblyCode[ctempBlock].m = cx; //set where jmp should go
	//symbolTable[indexTemp].codeAddr = cx + 1;
	initIdent(varCtr);//emit INC for the according as needed for the AR + locals
	list = statement(list);
	emit(RTN, 0, 0, 0);
	/*int j;
	for(j = nextAvailableIndex-1; j>=0; j++){
		if(symbolTable[j].symbol == 3){
			indexTemp = j;
			break;
		}
	}*/
...
lexNode* statement(lexNode* list){
	...
	if(list->tokType == identsym){
		...
		emit(STO, rp, lexLevel - symbolTable[i].lexLvl, symbolTable[i].stkAddr-1); //store whats at the top of the RF to the appriopriate variable in the stack
		rp--;
		...
		i = lookUp(list->lex);
		//FV parameters - 0
		//SL The static link points to the bottom of the ARI of the callee's static parent - 
		//DL The dynamic link points to the top of the ARI of the caller
		//RA 
		if(symbolTable[i].symbol != 3)
			error(15);
		//stkPos += 4; 
		emit(CALL, 0, lexLevel - symbolTable[i].lexLvl + 1, assemblyCode[symbolTable[i].codeAddr].m);
		list = nextToken(list);
	}
		...
		list = nextToken(list);
		ctemp = cx;			//CHECK IF ELSE WORKS
		emit(JPC, rp, 0, 0);//ctemp = JPC loc
		list = statement(list);
		if(list->tokType == elsesym){
			ctemp2 = cx;
			emit(JMP, 0,0,0);//ctemp2 = JMP loc last line of if block
			assemblyCode[ctemp].m = cx;
			list = nextToken(list);
			list = statement(list);
			assemblyCode[ctemp2].m = cx;
		} else {
			assemblyCode[ctemp].m = cx;
		}
	...
	/*
	JPC rp 0 <to else block> (if is true)
	<if block>
	JMP 0  0 <after else block>
	<else block> (if is false)
	<after alse block>
	*/
	/*
	else if(list->tokType == ifsym){
	...
	*/
		...
		if(symbolTable[i].symbol == 1){//integer
			rp++;
			emit(LOD, rp, lexLevel - symbolTable[i].lexLvl, symbolTable[i].stkAddr-1);	//integers get their values from the stack
		...
		if(symbolTable[i].symbol == 1){//found valid int read x; <keyboard input> -> x. <input> -> regStack; regStack -> x;
			rp++;
			emit(SIOREAD, rp, 0, 2);
			emit(STO, rp, lexLevel - symbolTable[i].lexLvl, symbolTable[i].stkAddr-1);
		}else{
...
lexNode* factor(lexNode* list){
	...
	if(list->tokType == identsym){
		...
		if(symbolTable[i].symbol == 1) //if its an int
			emit(LOD, rp, lexLevel - symbolTable[i].lexLvl, symbolTable[i].stkAddr - 1); //-1 because bp = 1
...
lexNode* nextToken(lexNode* list){
	list = list->next;
	if(PARSERDBG) printf("list->lex: %s\n", list->lex);
...
void initIdent(int varCtr){
	emit(INC, 0, 0, varCtr + 4); //Needs a +4 for activation record
	emit(LIT, 0, 0, 0);
	int i = 0;
	for(i = 0; i < varCtr; i++){
		emit(STO, 0, 0, topStk);
	...
	topStk = 5; //Reset back to top of an AR for the next procedures idents
}
...
void error(int e){
	if(aArg){
		...
		switch(e){
			...
			break;
			case 15:
			printf("Procedure identifier expected.\n");
			break;
			case 16:
