...
int main(int argc, char **argv){
	...
	//read until the end of file is found and create the root and subtries
	while(fscanf(ifp, "%s", str)!=EOF){
		//reset sentence array after each read
		for(i = 0;i<NUMWORDS;i++)
			for(j=0;j<NUMCHARS;j++)
				sentence[i][j]='\0';
		i=0;
		//copy strings into sentence array until "." is found
		do{
		...
		strcpy(sentence[i],str);
		root = createSentence(root, sentence);
	}
...
TrieNode *createSentence(TrieNode *root,char **sentence){
	int ctr = 0,i=0,totWords;
	TrieNode **subTrieHolder;
	if(SENTENCEDBG)printf("createSentence(): Creating main tree\n");
	while(strcmp(sentence[ctr],".") !=0 || ctr<NUMWORDS){
...
/*
	...
	subTrieHolder = malloc(sizeof(TrieNode *) *totWords);
	printf("%d\n",totWords);
