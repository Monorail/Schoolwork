...
public class Treap<AnyType extends Comparable<AnyType>> {
	...
	public static void main(String[] args) {
		...
		myTree.add("HII");
		myTree.add("HIII");
		System.out.println(myTree.getRoot().data);
	...
	public void add(AnyType data){
		Random r = new Random();
		int rVal = Math.abs(r.nextInt()%3000000);
		while(priorities.contains(rVal)){
			rVal = Math.abs(r.nextInt()%3000000);
		}
		add(data, rVal);
	}
	...
	public Node<AnyType> recurAdd(Node<AnyType> intRoot, AnyType data, int priority){
		System.out.println("Start");
		if(intRoot == null){
			System.out.println("Inserting [" + data+ "] with priority [" + priority + "]");
